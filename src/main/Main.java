...
public class Main {
    ...
    void login(String user, String password){
        ...
        for(i=0;i<admin.am.getDataAdmin().size();i++){
            //mengecek pass & id
         try{
         if(user.equals(admin.am.getDataAdmin().get(i).getUser()) && 
    ...
    void insert() {
        ...
        buku.insert(new BukuEntity (judulBuku, penulis, penerbit, kodeBuku, tahunTerbit, jumlahBuku));
    ...
    void update() {
        if(buku.bm.getDataBuku().size()>0){
            ...
            buku.update(selectIndex, new BukuEntity (judulBuku, penulis, penerbit, kodeBuku, tahunTerbit, jumlahBuku));
...
//                    viewPeminjaman();
    ...
    void peminjaman(){
        if(user.um.getDataUser().size()>0 && buku.bm.getDataBuku().size()>0){
            String kodeUser;
            String kodeBuku;
            String pilih="";
            System.out.println("Masukkan kode user = ");
            kodeUser = input.next();
            int indexUser = user.cekUser(kodeUser);
            user.cekUser(kodeUser);
            if(user.cekUser(kodeUser) == -1){
                ...
                do{
                    ...
                    if(buku.cekBuku(kodeBuku) == -1){
                        ...
                        System.out.println("\n === Data Peminjam === ");
                        System.out.println("Judul Buku : "+buku.showDaftarBuku(indexBuku).getJudulBuku());
                        //output lengkap 
                        System.out.println("Nama Peminjam : "+user.showDaftarUser(indexUser).getNama());
                        
                    }
                }while(pilih == "y");
                //if, jika cek kode buku != -1 ==> isi menginputkan tanggan dan jumlah, lalu simpan/tidak
            }
        } else{
